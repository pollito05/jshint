...
var OPTIONS = {
  ...
  "exclude": ["exclude",
    "Exclude files matching the given filename pattern (same as .jsmikeignore)", "string", null],
  "exclude-path": ["exclude-path", "Pass in a custom jsmikeignore file path", "string", null],
  "verbose": ["verbose", "Show message codes"],
  "show-non-errors": ["show-non-errors", "Show additional data generated by jsmike"],
  "extra-ext": ["e",
...
function findConfig(file) {
  ...
  var home = path.normalize(path.join(envs, ".jsmikerc"));

  var proj = findFile(".jsmikerc", dir);
  if (proj)
    return proj;
...
/**
 * Tries to find JSMike configuration within a package.json file
...
function loadNpmConfig(file) {
  ...
  try {
    return require(fp).jsmikeConfig;
  } catch (e) {
    return null;
  }
}
 ...
 * or hits the root.
 *
 * @param {string} name filename to search for (e.g. .jsmikerc)
...
/**
 * Loads a list of files that have to be skipped. JSMike assumes that
 * the list is located in a file called '.jsmikeignore'.
 *
 * @return {array} a list of files to ignore.
 */
function loadIgnores(exclude, excludePath) {
  var file = findFile(excludePath || ".jsmikeignore");
...
/**
 * Runs JSMike against provided file and saves the result
 *
 * @param {string} code    code that needs to be linted
 * @param {object} results a pointer to an object with results
 * @param {object} config  an object with JSMike configuration
...
function lint(code, results, config, data, file) {
  ...
  var report = jsmike.run(buffer, config, globals);
  if (!report.success) {
...
var exports = {
   ...
   *                   args     - CLI arguments
   *                   ignores  - A list of files/dirs to ignore (defaults to .jsmikeignores)
   ...
   * its done. This function is called from bin/jsmike file.
   *
  ...
  interpret: function (args) {
    ...
    // Reporter that displays additional JSMike data
